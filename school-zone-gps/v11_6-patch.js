/* v11.6 UI patch: expose why a route failed instead of generic red text. */
(function(){
  ...
  avoidRoute=async function(a,b,alternatives){
    ...
    if(!out.r&&out.diagnostic){
      ...
      setTimeout(()=>{
        try{
          $('detail').textContent=(out.diagnostic||'Routing failed.')+originNote;
          $('st').textContent='Could not validate a route: '+(out.diagnostic||'all available routes still cross a school zone.')+originNote;
        }catch(_){}
      },0);
    }
    return out;
