...
async function mergeSchoolsForRoute(r){try{const extra=await loadSchools(r);schools=uniqueSchools([...schools,...extra])}catch(e){console.warn('Additional school lookup unavailable',e)}}
async function brouterAvoid(a,b,seedRoute){
  ...
  for(let pass=0;pass<5;pass++){
    ...
    try{
      ...
      for(const h of hits){for(const s of [h.s,...schoolNeighbors(h.s,schools,1000)]){if(originIds.has(s.id))continue;if(blocked.length>=75)break;if(!blocked.some(x=>x.id===s.id)){blocked.push(s);added++}}}
...
/* ---- v11.7 SAFE-STOP FALLBACK -----------------------------------------
   If a full destination route cannot be validated, find the last blocking
   school zone and route to the closest reachable drivable point immediately
   OUTSIDE its 1,000 ft boundary. The endpoint is independently revalidated. */
const SAFE_STOP_MARGIN_M=15;
function routeEndpoint(r){const c=r?.geometry?.coordinates||[];if(!c.length)return null;const p=c[c.length-1];return{lon:+p[0],lat:+p[1]}}
function pointOutsideAllSchoolZones(p,extra=4){if(!p)return false;for(const s of schools){if(distanceMeters(p,{lat:s.lat,lon:s.lon})<=VALIDATION_RADIUS_M+extra)return false}return true}
function safeStopBlockingSchools(baseRoute,b){
  const out=[];const dc=destinationConflict(b);if(dc)out.push(dc.s);
  const hits=schoolHitInfos(baseRoute||{});for(let i=hits.length-1;i>=0&&out.length<5;i--)out.push(hits[i].s);
  const near=schools.map(s=>({s,d:distanceMeters(b,{lat:s.lat,lon:s.lon})})).filter(x=>x.d<5000).sort((x,y)=>x.d-y.d).slice(0,4).map(x=>x.s);
  out.push(...near);return uniqueSchools(out).slice(0,6);
}
function safeStopCandidates(s,b){
  const toward=bearing({lat:s.lat,lon:s.lon},b);
  const deltas=[0,10,-10,20,-20,30,-30,45,-45,60,-60,90,-90,120,-120,150,-150,180];
  const margins=[SAFE_STOP_MARGIN_M,35,75];const out=[];
  for(const margin of margins){for(const d of deltas){const p=destinationPoint(s.lon,s.lat,VALIDATION_RADIUS_M+margin,toward+d);if(!pointOutsideAllSchoolZones(p,3))continue;out.push({school:s,point:p,rawDistance:distanceMeters(p,b),margin})}}
  return out;
}
function markSafeStopRoute(r,b,s){
  const end=routeEndpoint(r);if(!end||!pointOutsideAllSchoolZones(end,2))return null;if(schoolHitInfos(r).length)return null;
  r._safeApproach={original:b,point:end,school:s.name,boundaryFallback:true,distanceToDestination:distanceMeters(end,b),distanceFromSchool:distanceMeters(end,{lat:s.lat,lon:s.lon}),note:'Closest reachable road point outside the mapped 1,000 ft school zone'};
  return r;
}
async function osrmSafeStopAttempt(a,b,candidate){
  try{
    const alts=await route(a,candidate.point,[],true);if(!alts?.length)return null;await mergeSchoolsForRoute(alts[0]);
    const clean=alts.map(r=>markSafeStopRoute(r,b,candidate.school)).filter(Boolean).sort((x,y)=>distanceMeters(routeEndpoint(x),b)-distanceMeters(routeEndpoint(y),b)||x.duration-y.duration);
    if(clean.length)return{route:clean[0],alts};return{route:null,alts};
  }catch(e){return{route:null,alts:null,error:e}}
}
async function findClosestReachableSafeStop(a,b,baseRoute){
  const blockers=safeStopBlockingSchools(baseRoute,b);if(!blockers.length)return null;
  let candidates=[];for(const s of blockers)candidates.push(...safeStopCandidates(s,b));
  candidates.sort((x,y)=>x.rawDistance-y.rawDistance||x.margin-y.margin);candidates=candidates.slice(0,24);
  progress(93,'Finding closest safe stop','Testing the red-zone boundary nearest the destination');
  const brouterQueue=[],successes=[];
  for(let i=0;i<candidates.length;i+=3){
    const batch=candidates.slice(i,i+3);const done=await Promise.all(batch.map(async c=>({c,res:await osrmSafeStopAttempt(a,b,c)})));
    for(const x of done){if(x.res?.route){successes.push(x.res.route)}else if(x.res?.alts?.[0]&&brouterQueue.length<8){brouterQueue.push({c:x.c,seed:x.res.alts[0]})}}
    if(successes.length){successes.sort((x,y)=>x._safeApproach.distanceToDestination-y._safeApproach.distanceToDestination||x.duration-y.duration);if(i>=5)return successes[0]}
    await new Promise(r=>setTimeout(r,0));
  }
  if(successes.length){successes.sort((x,y)=>x._safeApproach.distanceToDestination-y._safeApproach.distanceToDestination||x.duration-y.duration);return successes[0]}
  for(let i=0;i<brouterQueue.length;i++){
    const {c,seed}=brouterQueue[i];progress(95+Math.min(4,i*.5),'Refining safe stop',`Native no-go attempt ${i+1}/${brouterQueue.length}`);
    try{const br=await brouterAvoid(a,c.point,seed);const marked=br&&markSafeStopRoute(br,b,c.school);if(marked)return marked}catch(e){routingDiagnostics.detail=e.message}
  }
  return null;
}
...
async function avoidRoute(a,b,alternatives){
  ...
  for(const r of sorted){if(!schoolHitInfos(r).length){routingDiagnostics.engine='OSRM';routingDiagnostics.detail=routingDiagnostics.originZones?`Normal route valid after exiting ${routingDiagnostics.originZones} origin school zone${routingDiagnostics.originZones===1?'':'s'}`:'Normal route already has zero school-zone crossings';return{r,n:0}}}
  ...
  try{
    const safeStop=await findClosestReachableSafeStop(a,b,sorted[0]);
    if(safeStop){routingDiagnostics.engine='Safe Stop fallback';routingDiagnostics.detail='Full destination route was blocked; routed to the closest reachable point outside the final school zone.';return{r:safeStop,n:0,safeApproach:true,boundaryFallback:true}}
  }catch(e){routingDiagnostics.detail=(routingDiagnostics.detail?routingDiagnostics.detail+' · ':'')+'Safe-stop search: '+e.message}
  return{r:null,n:Math.max(1,schoolHitInfos(sorted[0]).length),diagnostic:routingDiagnostics.detail||'No clean route or reachable safe boundary stop was returned by available routing engines',originZones:routingDiagnostics.originZones};
}
function drawSchools(r){
  ...
  for(const s of schools){
    if(s.lon<b.minLon-padLon||s.lon>b.maxLon+padLon||s.lat<b.minLat-padLat||s.lat>b.maxLat+padLat)continue;
    L.circle([s.lat,s.lon],{radius:VALIDATION_RADIUS_M,color:'#D93025',weight:2.5,fillColor:'#EA4335',fillOpacity:.13}).bindPopup('<b>'+s.name+'</b><br>1,000 ft school zone'+(originInsideSchool(s)?'<br><small>Trip starts inside this zone; exit-only exception used by routing logic.</small>':'')).addTo(schoolLayer);
  }
}
