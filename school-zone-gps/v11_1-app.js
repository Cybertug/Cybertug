/* v11.1 app controller - robust GPS/navigation */
...
async function build(fromGps=false){
  ...
  try{
    ...
    progress(100,'Route ready','Strict validation passed');$('fill').style.background='#006cff';status('Route validated. Tap Navigation for follow mode.','ok');loadRestrictedAsync(r,b)
...
async function startGpsTracking(){
  ...
  try{
    ...
    const first=await gpsOnce();const firstHeading=routeBearingNear(first.lat,first.lon);
    window.__setNavMarker?.(first.lon,first.lat,firstHeading||0);window.__updateNavCamera?.(first.lat,first.lon,firstHeading,0,false);
    watch=navigator.geolocation.watchPosition(p=>{const lat=p.coords.latitude,lon=p.coords.longitude,speed=Number.isFinite(p.coords.speed)?p.coords.speed:0,heading=Number.isFinite(p.coords.heading)&&p.coords.heading!=null?p.coords.heading:routeBearingNear(lat,lon);window.__updateNavCamera?.(lat,lon,heading,speed,false);if(AR){let off=0;try{off=turf.pointToLineDistance(turf.point([lon,lat]),turf.lineString(AR.geometry.coordinates),{units:'kilometers'})*1000}catch(_){}if(off>130)reroute(lat,lon)}},e=>{stopGps();status(e.code===1?'GPS permission denied. Allow Location for this site, then tap Start GPS again.':'GPS error: '+e.message,'err')},{enableHighAccuracy:true,maximumAge:1000,timeout:15000});
    $('gps').textContent='GPS Active';$('gps').classList.add('active');status(window.__SZ_MAP_MODE==='vector'?'GPS active · heading-up follow mode.':'GPS active · stable follow mode.','ok');return true
...
$('navBtn').onclick=async()=>{if(!AR)return status('No validated route is available for navigation.','err');if(hitList(AR).length)return status('Navigation blocked: route failed school-zone validation.','err');$('sum').style.display='none';$('nav').style.display='flex';window.__enterNavVisual?.();$('navTitle').textContent=window.__SZ_MAP_MODE==='vector'?'Navigation':'Navigation · Stable Mode';$('navText').textContent=window.__SZ_MAP_MODE==='vector'?'Following position + heading':'Following live GPS position';await startGpsTracking()};
